...
(0.6-0.8)/0.8 * 100
```
You're often interested in looking at how did the output change relative to the change in the input. Here it is super proportional.
...
```{r}
uncertainty <- rnorm(mean = 10000, sd = 1500, n = 100)

# use map from purrr to run model for all values of eff
# notice how map adds the one parameter that is missing from the input list

site2 <- uncertainty %>% map(~ solarpv(area = 0.1, 
  solar = sierraczosolar, g = FALSE, etype = "direct", ethresh = .x))
