...
-   dynamic? Incorporate stream flow into reservoir (depends on time forcing), height depends on flow out of reservoir, different flow might depend on energy need
-   spatially distributed? including more area, more reservoirs or more of watershed
-   stochastic? estimate probability of power production based on probability distribution of water levels
...
**Example Q1**: Will a forest fire in the upper 1/2 of a watershed impact stream flow more than a similarly sized fire lower (nearer the stream) in the watershed?

this would need to be a spatial model and estimate how fire could possibly affect stream flow (ex: trees and plants intercept and absorb water- need to represent the relationship between vegetation and water use)
